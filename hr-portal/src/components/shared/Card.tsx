...
export function Card({ title, children, className = '' }: { title?: string; children: ReactNode; className?: string }) {
  return <div className={`bg-white rounded-xl border border-slate-200 p-6 ${className}`}>
    {title && <h3 className="text-lg font-semibold text-slate-900 mb-4">{title}</h3>}{children}
  </div>
}
...
export function Badge({ variant = 'primary', children }: { variant?: 'primary' | 'success' | 'warning' | 'danger' | 'info'; children: React.ReactNode }) {
  const c: Record<string, string> = { primary: 'bg-orange-50 text-orange-700', success: 'bg-green-50 text-green-700', warning: 'bg-yellow-50 text-yellow-700', danger: 'bg-red-50 text-red-700', info: 'bg-blue-50 text-blue-700' }
